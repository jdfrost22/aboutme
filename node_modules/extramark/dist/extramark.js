var ExtraMark = (function (exports) {
	'use strict';
	...
	const decode$2 = function decode(input) {
	  // Don't use UCS-2.
	  const output = [];
	...
	const encode$2 = function encode(input) {
	...
	var n = {
	  ...
	  t = Object.assign({}, r.defaults, t), n.core.ruler.push("anchor", function (n) {
	    var r,
	        i = {},
	        u = n.tokens,
	        o = Array.isArray(t.level) ? (r = t.level, function (n) {
	      return r.includes(n);
	    }) : function (n) {
	      return function (e) {
	        return e >= n;
	...
	r.defaults = {
	  ...
	  renderPermalink: function renderPermalink(e, r, t, i) {
	    var u,
	      ...
	      attrs: [["class", r.permalinkClass], ["href", r.permalinkHref(e, t)], ["aria-hidden", "true"]]
	...
	const addition = {
	  ...
	  render(_, match) {
	    ...
	    return "<ins>".concat(match, "</ins>");
	  }

	};

	const deletion = {
	  regex: /\{\x2D\x2D([\s\S]*?)\x2D\x2D\}/g,
	  validate: () => true,
	...
	const comment$1 = {
	  ...
	  validate: (_ref) => {
	    ...
	    return input.substr(index - ignorePattern.length, ignorePattern.length) !== ignorePattern;
	  },
	  ...
	  render(_, match) {
	    return "<span class=\"critic comment\">".concat(match, "</span>");
	  }
	...
	const highlight = {
	  ...
	  render(_, match1, match2) {
	    return "<mark>".concat(match1, "</mark>").concat(comment$1.render(_, match2));
	  }

	};

	function render(text) {
	  return text.replace(substitution.regex, substitution.render).replace(addition.regex, addition.render).replace(deletion.regex, deletion.render).replace(highlight.regex, highlight.render).replace(comment$1.regex, comment$1.render);
	}
	...
	parser.use(r, [1, 2, 3]).use(markdownItDeflist).use(markdownItSup).use(markdownItSub).use(markdownItAbbr).use(markdownItFootnote).use(markdownItCriticMarkup);
	const parse$2 = parser.parse.bind(parser);
	const render$1 = parser.render.bind(parser);
...
//# sourceMappingURL=extramark.js.map
